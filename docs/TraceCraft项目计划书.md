...
**状态**：V1.0 MVP 已入库（后台原型 + 前端 Demo 跑通）；产品策略已收窄为“城市创意跑路线 + 跑团挑战工具”  
...
- **可跑性检查**（半自动）：V1 先输出风险提示和人工确认；后续接入路网后自动避开不可通行区域。
...
4. 选 3 个城市可控场景做路线样本：公园/绿道、校园、低车流街区，每个场景生成 1 条基础图形路线。  
5. 找 3-5 个目标用户快速访谈：跑团组织者、内容创作者、普通城市跑者，确认他们是否愿意真实开跑和分享。  
6. 编写 `docs/TraceCraft-API-Contract.md`（routes、run、maps）用于 RN 迁移。  
7. 制作 iOS/Android 权限文案与隐私页（位置权限、数据留存、匿名化策略）。

---

## 附录：当前实现现状（可落地）
- 已落地：route/session 持久化（文件态）、定位字段、`providerHint/crsHint`、`rebase`、`maps/config`、`start-run/location/state/finish`。  
- 待落地：RN 原生页面、API key 的正式环境化、Redis/PostgreSQL、百度/腾讯真实地图适配器 SDK、真实图片识别、城市路网可跑性约束。  
