...
1. 明确“基础模板”和“自定义图片”的真实业务链路：都必须生成路线点，不直接跳导航。
2. 将“路线预览 + 风险提示 + 用户确认”定义为导航前置关口。
3. 更新计划文档和 README，补齐风险分级、弹窗动作、接口返回字段和后续排期。

### 本次交付结果

| 模块 | 事项 | 状态 | 备注 |
| --- | --- | --- | --- |
| 产品流程 | 基础模板/自定义图片统一进入路线预览确认 | 已完成 | 已落地前端预览页 |
| 风险策略 | GPS 精度、起点距离、距离偏差、路线点异常跨度、高德步行规划抽样 | 已完成 | 低/中/高风险分级 |
| 交互策略 | 弹窗操作：调整起点 / 重新生成 / 仍然使用 | 部分完成 | 预览页已支持确认，高风险阻断；细化弹窗样式后续补 |
| API 契约 | 补充 `riskLevel`、`riskSegments`、`startPointStatus`、`confirmRequired` | 已完成 | 后端接口已返回 |
| 地图服务 | V1 优先接高德 Web 服务步行规划风险抽样 | 已完成 | 百度 Key 预留为后续 fallback |
| 文档 | 更新 README 与项目计划书 | 已完成 | 已同步实现口径 |

### 未完成/待处理

| 模块 | 待办 | 目标日期 | 风险 |
| --- | --- | --- | --- |
| 前端 | 路线预览页接真实地图 SDK 渲染，而非 SVG 预览 | 2026-06-21 | 中 |
| 前端 | 参数调整页接 `/adjust` 与 `/rebase` 后返回预览页 | 2026-06-20 | 中 |
| 后端 | 百度步行规划 fallback | 2026-06-24 | 中 |
| 产品 | 路线风险弹窗文案和阻断规则细化到 UI 文档 | 2026-06-17 | 低 |
| 验证 | 补充模板路线、自定义图片、风险阻断三条手工验收用例 | 2026-06-20 | 中 |

### 版本与里程碑映射
- 关键改动：产品链路从“选择模板后直接进入参数/导航”调整为“生成路线点 -> 预览风险 -> 用户确认 -> 导航”。
- 下次同步建议：实现前先补 `docs/TraceCraft-API-Contract.md`，固定风险字段和前端状态机，避免前后端口径漂移。
