...
## 11. 最新进度表（2026-06-25，限流中间件抽取 + 法务文档屏幕 + CSRF 精简）

### 本次同步目标（限流中间件抽取 + 法务文档屏幕 + CSRF 精简）

1. 后端限流逻辑抽取为独立中间件模块 `rateLimit.ts`，消除各路由文件重复的 RedisStore 配置
2. 后端 CSRF 中间件移除 multipart 跳过逻辑（multer 已先行解析，无需额外 bypass）
3. Admin 前端 API 客户端接入 CSRF Token 自动携带
4. 前端新增法务文档独立屏幕（隐私政策 / 用户协议 / 权限说明），替代登录页内嵌弹窗
5. 前端新增 `LegalDocumentScreen` + 本地法务文案数据模块（中英双语）
6. 前端设置页新增「权限使用说明」入口，隐私政策和用户协议改为跳转独立屏幕
7. ScreenId 类型扩展 + 导航钩子更新

### 本次交付结果

| 模块 | 事项 | 状态 | 备注 |
| --- | --- | --- | --- |
| 后端 | `createRedisAwareRateLimiter` 独立中间件 | 已完成 | 惰性创建，首次请求时构建 limiter |
| 后端 | `index.ts` / `authApi.ts` / `routeApi.ts` 统一使用新中间件 | 已完成 | 移除各文件直接 `rateLimit()` + `RedisStore` |
| 后端 | CSRF 移除 multipart bypass | 已完成 | 简化逻辑，multer 已先解析 |
| 后端 | CSRF 测试更新（multipart 场景） | 已完成 | 新增 2 个 multipart 测试用例 |
| Admin | API 客户端接入 CSRF Token | 已完成 | 自动读取 Cookie / 请求 `/csrf-token` |
| 前端 | `LegalDocumentScreen` 独立屏幕 | 已完成 | 支持远程内容 + 本地降级 |
| 前端 | `legalContent.ts` 法务文案数据（中英双语） | 已完成 | 隐私政策/用户协议/权限说明 |
| 前端 | `LoginScreen` 移除内嵌弹窗 | 已完成 | 改为跳转独立屏幕 |
| 前端 | `SettingsScreen` 新增权限说明入口 | 已完成 | 隐私/协议也改为跳转独立屏幕 |
| 前端 | `ScreenId` 新增 3 个法务屏幕 ID | 已完成 | privacy_policy / user_agreement / permission_notice |
| 前端 | `useScreenNavigation` 导航钩子更新 | 已完成 | 法务屏幕归属 profile Tab |

### 新增文件

| 文件 | 说明 |
| --- | --- |
| `backend/src/middleware/rateLimit.ts` | Redis 感知限流中间件工厂（惰性创建） |
| `frontend/src/components/screens/LegalDocumentScreen.tsx` | 法务文档独立屏幕组件 |
| `frontend/src/data/legalContent.ts` | 本地法务文案（中英双语，3 份文档） |
...
| `backend npm test` | 59 测试全部通过（0 失败） |

## 12. 历史进度表（2026-06-24，Zod 验证全面接入 + PWA 与 Docker 完善）

### 本次同步目标（Zod 验证全面接入 + PWA 与 Docker 完善）

1. 后端 9 个路由端点接入 Zod 请求体验证（社区/发现/路线）
2. 后端生产环境 CSP 显式配置 + PostgreSQL 连接池监控
3. 后端分享海报 Sharp 失败降级 + 占位 SVG 生成
4. 前端 PWA Service Worker 重写（API 缓存 + 版本管理）
5. 前端 ErrorBoundary 包裹 + 地图服务商切换 + 通知权限请求
6. 前端 Dockerfile + docker-compose 前端服务完善

### 本次交付结果

| 模块 | 事项 | 状态 | 备注 |
| --- | --- | --- | --- |
| 验证 | 社区 API 接入 Zod：createPost/addComment/reportPost/batchReadNotifications | 已完成 | 4 个端点 |
| 验证 | 发现 API 接入 Zod：setFavorite/searchQuery | 已完成 | 2 个端点 |
| 验证 | 路线 API 接入 Zod：from-template/adjust/rebase | 已完成 | 3 个端点 |
| 安全 | 生产环境 CSP 显式配置（Leaflet 瓦片/Socket.IO） | 已完成 | 替代开发环境宽松策略 |
| 监控 | PostgreSQL 连接池定期监控（30s 告警） | 已完成 | 接近耗尽时 warn 日志 |
| 容错 | 分享海报 Sharp 失败降级 + 占位 SVG | 已完成 | `safeGenerateWebp` 三级降级 |
| PWA | Service Worker 重写：API GET 缓存 + 版本管理 | 已完成 | 修复旧版重复代码 |
| 前端 | ErrorBoundary 包裹 App 根组件 | 已完成 | 崩溃时显示错误界面 |
| 前端 | 设置页地图服务商切换（amap/google/baidu） | 已完成 | 底部抽屉选择器 |
| 前端 | Web 通知权限请求（初始化时） | 已完成 | permission=default 时自动请求 |
| DevOps | Dockerfile.backend 增加 vips-dev 依赖 | 已完成 | sharp 编译需要 |
| DevOps | Dockerfile.frontend（Nginx + SPA 路由 + API 代理） | 已完成 | 多阶段构建 |
| DevOps | docker-compose.yml 新增 frontend 服务 | 已完成 | 依赖 backend healthy |

### 新增文件

| 文件 | 说明 |
| --- | --- |
| `Dockerfile.frontend` | 前端多阶段 Docker 构建（Vite → Nginx） |
...
## 13. 历史进度表（2026-06-23，第三方服务商接入与生产就绪增强）
...
## 14. 历史进度表（2026-06-23，TypeScript 严格模式增强与代码清理）
...
## 15. 历史进度表（2026-06-22，全栈安全加固与基础设施完善）
...
## 16. 历史进度表（2026-06-22，前端屏幕组件拆分与路由重构）
...
## 17. 历史进度表（2026-06-18，P0 安全中间件 + P2 功能增强集成）
...
## 18. 历史进度表（2026-06-18，开发服务端口重新分配）
...
## 19. 历史进度表（2026-06-18，全栈代码质量优化 + 未使用代码清理 + 中文注释增强）
...
## 20. 历史进度表（2026-06-17，管理后台 React + Ant Design 改造）
...
## 21. 历史进度表（2026-06-17，数据库接入 + 用户数据页真实化）
